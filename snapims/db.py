...
def _base_schema(connection: sqlite3.Connection) -> None:
    connection.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_import_photo_cache_folder ON import_photo_cache(folder_path, captured_at, original_name);
        CREATE UNIQUE INDEX IF NOT EXISTS idx_import_photo_cache_id ON import_photo_cache(cache_id) WHERE cache_id<>'';
        CREATE INDEX IF NOT EXISTS idx_import_jobs_folder ON import_jobs(folder_path, created_at DESC);
        ...
        CREATE INDEX IF NOT EXISTS idx_shopify_publish_jobs_batch
            ON shopify_publish_jobs(batch_id, created_at);
        CREATE INDEX IF NOT EXISTS idx_shopify_publish_jobs_status
            ON shopify_publish_jobs(status, updated_at);
        CREATE INDEX IF NOT EXISTS idx_shopify_publish_job_items_status
            ON shopify_publish_job_items(job_id, status, sequence);
        """
    )
...
EXPECTED_SCHEMA: dict[str, set[str]] = {
    ...
    "batches": {
        ...
        "reservation_eligible", "quarantine_reason", "display_name",
        "source_folder_name", "batch_location", "import_job_id",
    },
    "items": {
        "item_id", "sku", "batch_id", "sequence", "shelf", "location", "title", "price_cents",
    ...
    "import_photo_cache": {
        "source_path", "cache_id", "folder_path", "original_name", "file_size", "mtime_ns",
        "captured_at", "classification", "thumbnail_path", "sha256", "scanned_at",
        "decode_count", "qr_scan_count",
    },
    "import_photo_corrections": {
        "folder_path", "source_path", "action", "split_before", "merge_previous", "updated_at",
    },
    "import_jobs": {
        "job_id", "folder_path", "folder_name", "batch_name", "batch_location", "status",
        "stage", "processed_count", "total_count", "preview_json", "created_at", "updated_at",
        "decode_count", "qr_scan_count", "full_hash_count", "cache_hits", "cache_misses",
    },
    "operation_requests": {
    ...
    "shopify_sync": {
        "item_id", "status", "product_id", "variant_id", "inventory_item_id",
        "admin_url", "media_count", "last_synced_at", "last_error",
        "retry_count", "idempotency_key", "last_completed_step",
        "product_status", "sync_state", "last_action", "remote_updated_at",
        "archived_at", "deleted_at", "conflict_json",
    },
    "shopify_publish_jobs": {
        "job_id", "batch_id", "action", "status", "selection_scope", "total",
        "completed", "succeeded", "failed", "skipped", "current_item_id",
        "confirmation_text", "options_json", "error_message", "created_at",
        "started_at", "updated_at", "finished_at",
    },
    "shopify_publish_job_items": {
        "job_id", "item_id", "sequence", "action", "status", "attempt_count",
        "error_message", "result_json", "started_at", "updated_at", "finished_at",
    },
    "operational_events": {
...
EXPECTED_INDEXES = {
    ...
    "idx_shopify_publish_jobs_batch",
    "idx_shopify_publish_jobs_status",
    "idx_shopify_publish_job_items_status",
}
...
def schema_manifest_report(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    required_unique: dict[str, set[tuple[str, ...]]] = {
        ...
        "shopify_publish_jobs": {("job_id",)},
        "shopify_publish_job_items": {("job_id", "item_id")},
        "import_photo_cache": {("source_path",)},
        "import_photo_corrections": {("folder_path", "source_path")},
        "import_jobs": {("job_id",)},
...
def _upgrade_legacy(connection: sqlite3.Connection) -> None:
    ...
    if "items" in tables:
        _add_column(connection, "items", "location TEXT")
    ...
    if "import_photo_cache" in tables:
        _add_column(connection, "import_photo_cache", "cache_id TEXT NOT NULL DEFAULT ''")
        rows = connection.execute(
            "SELECT source_path FROM import_photo_cache WHERE cache_id=''"
        ).fetchall()
        for row in rows:
            ...
            connection.execute(
                "UPDATE import_photo_cache SET cache_id=? WHERE source_path=?",
                (cache_id, source_path),
            )
    ...
    if existing:
        ...
        if current == SCHEMA_VERSION:
            with transaction(db_file) as connection:
                cache_columns = {
                    str(row[1])
                    for row in connection.execute("PRAGMA table_info(import_photo_cache)").fetchall()
                }
                if cache_columns and "cache_id" not in cache_columns:
                    _add_column(connection, "import_photo_cache", "cache_id TEXT NOT NULL DEFAULT ''")
                    rows = connection.execute("SELECT source_path FROM import_photo_cache").fetchall()
                    for row in rows:
                        source_path = str(row[0])
                        cache_id = hashlib.sha256(
                            source_path.encode("utf-8", errors="surrogatepass")
...
def update_item_in_connection(
    ...
    *,
    ...
    location_changed = (
        ("location" in updates and updates["location"] != previous["location"])
        or ("shelf" in updates and updates["shelf"] != previous["shelf"])
    )
    if location_changed and not reason.strip():
        raise ValueError("A reason is required when changing Item location.")
    timestamp = now()
    ...
    if location_changed:
        old_location = previous["location"] if previous["location"] not in (None, "") else previous["shelf"]
        new_location = updates.get("location")
        if new_location in (None, "") and "location" not in updates:
            new_location = updates.get("shelf", previous["shelf"])
        connection.execute(
    ...
    from snapims.pricing.integration import record_item_change_in_connection
    record_item_change_in_connection(
        connection, item_id, previous, saved, source=source
    )
    return dict(saved)
