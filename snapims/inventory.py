...
def validation_errors(item: dict[str, Any], photos: list[dict[str, Any]]) -> list[str]:
    ...
    location_value = item.get("location") if "location" in item else item.get("shelf")
    if location_value not in (None, ""):
        ...
        if any(ord(character) < 32 for character in location_text):
            errors.append("Location contains unsupported control characters")
