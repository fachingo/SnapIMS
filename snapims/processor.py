...
def process_batch(
    ...
    *,
    ...
    batch = prepared_batch or parse_batch(
        source_folder,
    ...
    if progress_callback is not None:
        progress_callback("Preparing durable import", 0, len(batch.source_photos))
    ...
    for completed, artifact in enumerate(artifacts, start=1):
        ...
        if progress_callback is not None:
            progress_callback("Preserving and deriving photographs", completed, len(artifacts))
    ...
    if progress_callback is not None:
        progress_callback("Writing manifests", len(artifacts), len(artifacts))
