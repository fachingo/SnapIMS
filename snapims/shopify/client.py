...
class HTTPShopifyTransport:
    def __init__(self, config: ShopifyConfig, timeout: int = 60):
        self.config = config
        self.timeout = timeout
        self.endpoint = (
            f"https://{config.store_domain}/admin/api/{config.api_version}/graphql.json"
        )
...
class ShopifyClient:
    def __init__(
        self, config: ShopifyConfig, transport: ShopifyTransport | None = None
    ) -> None:
        self.config = config
    ...
    def _call(self, query: str, variables: dict[str, Any], field: str) -> dict[str, Any]:
        ...
        if user_errors:
            raise ShopifyAPIError(
                "; ".join(error.get("message", "Shopify user error") for error in user_errors)
            )
        return data

    def shop_identity(self) -> dict[str, Any]:
        payload = self.transport.graphql(
            "query ShopIdentity { shop { id name myshopifyDomain } }", {}
        )
        if payload.get("errors"):
            raise ShopifyAPIError(payload["errors"][0].get("message", "GraphQL error"))
        return payload["data"]["shop"]

    def locations(self) -> list[dict[str, Any]]:
        payload = self.transport.graphql(
            "query Locations { locations(first: 50) { nodes { id name isActive } } }", {}
        )
        if payload.get("errors"):
            raise ShopifyAPIError(payload["errors"][0].get("message", "GraphQL error"))
        return payload["data"]["locations"]["nodes"]
        ...
        payload = self.transport.graphql(
            ...
            query VariantBySku($query: String!) {
              productVariants(first: 2, query: $query) {
                nodes { id sku inventoryItem { id } product { id title status } }
              }
            }
            """,
            {"query": f"sku:{sku}"},
    ...
    def create_draft_product(self, item: dict[str, Any]) -> dict[str, str]:
        data = self._call(
            ...
            {
                "product": {
                    "title": item["title"], "descriptionHtml": item["description"],
                    "vendor": item["vendor"], "productType": item["product_type"],
    ...
    def configure_variant(self, item: dict[str, Any], product_id: str, variant_id: str) -> None:
        variant: dict[str, Any] = {
            ...
            "taxable": True,
            "inventoryItem": {
                "sku": item["sku"], "tracked": True, "requiresShipping": True,
            },
        }
        if item["barcode"]:
        ...
        self._call(
            """
            mutation ConfigureVariant($productId: ID!, $variants: [ProductVariantsBulkInput!]!) {
              productVariantsBulkUpdate(productId: $productId, variants: $variants) {
                productVariants { id inventoryItem { id tracked sku } price barcode }
                userErrors { field message }
              }
            }
            """,
    ...
    def activate_inventory(
        ...
        self._call(
            ...
            mutation ActivateInventory(
              $inventoryItemId: ID!, $locationId: ID!, $available: Int!, $key: String!
            ) {
              inventoryActivate(
    ...
    def upload_staged_images(
        self, image_paths: list[Path], targets: list[dict[str, Any]]
    ) -> list[str]:
    ...
    def attach_media(self, product_id: str, resource_urls: list[str], title: str) -> None:
        media = [
            {
                "originalSource": url, "alt": f"{title} - image {index}",
        ...
        self._call(
            ...
            mutation AttachMedia($product: ProductUpdateInput!, $media: [CreateMediaInput!]) {
              productUpdate(product: $product, media: $media) {
                product { id media(first: 100) { nodes { id preview { status } } } }
                userErrors { field message }
              }
            }
            """,
