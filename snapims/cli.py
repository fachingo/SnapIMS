...
from snapims.config import SnapIMSConfig, ShopifyConfig
from snapims.shopify.auth import ShopifyTokenManager
from snapims.provider_probes import ShopifyConnectionProbe
from snapims.shopify.jobs import list_jobs as list_shopify_jobs
...
def build_parser() -> argparse.ArgumentParser:
    ...
    shopify = sub.add_parser("shopify", help="Inspect and test the Shopify connection")
    shopify_sub = shopify.add_subparsers(dest="shopify_command", required=True)
    shopify_sub.add_parser("status", help="Show saved connection state without writing to Shopify")
    shopify_sub.add_parser("test", help="Run a read-only live identity, scope, location, and publication probe")
    shopify_sub.add_parser("refresh", help="Refresh the short-lived access token now")
    shopify_jobs = shopify_sub.add_parser("jobs", help="List recent durable Shopify jobs")
    shopify_jobs.add_argument("--batch", default="")
    shopify_jobs.add_argument("--limit", type=int, default=20)
...
def main() -> None:
    ...
    os.environ["SNAPIMS_DATA_DIR"] = str(config.data.root)
    os.environ["SNAPIMS_PROJECT_PATH"] = str(config.project_path)
    paths = config.data
    ...
    if args.command == "up":
        ...
        shopify_token = ShopifyTokenManager().status()
        shopify_ready = not shopify_config.problems()
        print(
            ...
            f"Shopify .............. {_pass_fail(shopify_ready)}\n"
            f"Shopify auth ......... {shopify_token.mode} / {shopify_token.state}\n"
            f"Port ................. {status['port']}\n"
        ...
        if args.shopify_command == "status":
            token = token_manager.status()
            print(f"Store domain ......... {shopify_config.store_domain or '(not configured)'}")
            print(f"Authentication ....... {shopify_config.auth_mode}")
            print(f"Token state .......... {token.state}")
            print(f"Token expires ........ {token.expires_at or '(not cached)'}")
            print(f"Inventory location ... {shopify_config.location_id or '(not selected)'}")
            print(f"Publication .......... {shopify_config.publication_id or '(not selected)'}")
            problems = shopify_config.problems()
            print(f"Configuration ........ {'PASS' if not problems else 'FAIL'}")
            for problem in problems:
                print(f"  - {problem}")
        elif args.shopify_command == "test":
            result = ShopifyConnectionProbe(
                shopify_config, token_manager=token_manager
            ).run()
            print(f"Store ................. {result.shop.get('name', '')}")
            print(f"Permanent domain ...... {result.shop.get('myshopify_domain', '')}")
            print(f"Shop ID ............... {result.shop.get('id', '')}")
            print(f"Authentication ........ {result.authentication_mode}")
            print(f"Token state ........... {result.token_state}")
            print(f"Token expires ......... {result.token_expires_at or '(unknown)'}")
            print("Required capabilities:")
            for capability, missing in result.missing_by_capability.items():
                print(f"  {'READY' if not missing else 'MISSING'}  {capability}" +
                      ("" if not missing else f" ({', '.join(missing)})"))
            print("Locations:")
            for location in result.locations:
                print(f"  {location.get('name')} · {location.get('id')}")
            print("Publications:")
            for publication in result.publications:
                print(f"  {publication.get('name')} · {publication.get('id')}")
            if result.missing_scopes:
                raise SystemExit(3)
        elif args.shopify_command == "refresh":
            token_manager.get_access_token(force_refresh=True)
            status = token_manager.status()
            print("Shopify access token refreshed successfully.")
            print(f"Expires ............... {status.expires_at}")
        elif args.shopify_command == "jobs":
            rows = list_shopify_jobs(
                paths.db_file, args.batch, limit=max(1, min(args.limit, 200))
            )
            if not rows:
                print("No Shopify jobs recorded.")
            for row in rows:
                print(
                    f"{row['created_at']}  {row['status']:<22} {row['action']:<16} "
                    f"{row['completed']}/{row['total']} failed={row['failed']} "
                    f"batch={row['batch_id']} job={row['job_id']}"
                )
