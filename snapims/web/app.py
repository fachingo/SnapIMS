...
from snapims.import_v0130 import (
    ImportV0130Error,
    apply_correction as apply_import_correction,
    batch_home as configured_batch_home,
    get_job as get_import_job,
    list_batch_folders,
    normalize_location as normalize_import_location,
    recover_jobs as recover_import_jobs,
    start_commit as start_import_commit,
    start_preview as start_import_preview,
    thumbnail_path as import_thumbnail_path,
    update_batch_display_name,
)
from snapims.processor import reconcile_import_journals
...
from snapims.shopify.jobs import (
    ShopifyJobError,
    create_job as create_shopify_job,
    get_job as get_shopify_job,
    list_jobs as list_shopify_jobs,
    recover_jobs as recover_shopify_jobs,
    resume_job as resume_shopify_job,
)
...
from snapims.pricing.integration import (
    pricing_batch_mode,
    pricing_default_disposition,
    pricing_diagnostics,
    pricing_item_state,
    record_review_pricing,
)
from snapims.pricing.router import router as pricing_router
...
TEMPLATES.env.globals.update(
    pricing_batch_mode=pricing_batch_mode,
    pricing_default_disposition=pricing_default_disposition,
    pricing_diagnostics=pricing_diagnostics,
    pricing_item_state=pricing_item_state,
)
...
async def lifespan(app: FastAPI):
    ...
    shopify_job_recovery = recover_shopify_jobs(
        paths, start_workers=not bool(os.getenv("PYTEST_CURRENT_TEST"))
    )
    ...
    _event(
        ...
        detail={
            ...
            "folder_import_recovery": folder_import_recovery,
            "shopify_job_recovery": shopify_job_recovery,
    ...
    try:
        ...
        try:
            get_pricing_worker().shutdown()
        except Exception as exc:
            LOGGER.exception("Pricing worker shutdown reconciliation failed: %s", exc)
        _event(
            "app",
            "application.stopped",
            status="STOPPED",
...
async def authentication(request: Request, call_next):
    ...
    request.state.username = (
        str(claims.get("sub", "")) if claims else "local-operator"
    )
...
async def import_page(
    ...
    paths = get_paths()
    folders = list_batch_folders(paths)
    job = get_import_job(paths.db_file, job_id) if job_id else None
    preview = (job or {}).get("preview") or {}
    all_items = list(preview.get("items") or [])
    page_size = 20
    page_count = max(1, (len(all_items) + page_size - 1) // page_size)
    page = min(page, page_count)
    preview_items = all_items[(page - 1) * page_size : page * page_size]
    return TEMPLATES.TemplateResponse(
        request,
        "import.html",
        context(
            request,
            batch_home=str(configured_batch_home(paths)),
            folders=folders,
            job=job,
            preview=preview,
...
async def import_preview_v0130(
    folder_path: str = Form(...),
    batch_location: str = Form(""),
) -> RedirectResponse:
    try:
        job_id = start_import_preview(
            get_paths(),
            Path(folder_path),
            location=batch_location,
        )
        return redirect(f"/import?job_id={quote(job_id)}")
    except ImportV0130Error as exc:
        return redirect(f"/import?message={quote(str(exc))}")


@app.get("/import/job/{job_id}")
async def import_job_status(job_id: str) -> JSONResponse:
    job = get_import_job(get_paths().db_file, job_id)
    if not job:
        raise HTTPException(404, "Import job not found")
    return JSONResponse(job)


@app.get("/import/next-item-card.png")
async def import_next_item_card() -> StreamingResponse:
    image = qrcode.make("CVHS1:ITEM:NEXT").convert("RGB")
    output = BytesIO()
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="SnapIMS-NEXT-ITEM-v0.13.0.png"'},
    )


@app.get("/import/thumbnail/{cache_id}")
async def import_thumbnail(cache_id: str) -> StreamingResponse:
    path = import_thumbnail_path(get_paths().db_file, cache_id)
    if path is None:
        raise HTTPException(404)
    return safe_file_response(path, media_type="image/jpeg")


@app.post("/import/correction")
async def import_correction_v0130(
    job_id: str = Form(...),
    source_path: str = Form(...),
    action: str = Form("auto"),
    operation: str = Form("interpret"),
) -> RedirectResponse:
    try:
        split_before = operation == "split"
        merge_previous = operation == "merge"
        apply_import_correction(
            get_paths(),
            job_id,
            source_path,
            action=action,
        ...
        return redirect(f"/import?job_id={quote(job_id)}&notice={quote('Preview interpretation updated without rescanning photographs.')}")
    except ImportV0130Error as exc:
        return redirect(f"/import?job_id={quote(job_id)}&message={quote(str(exc))}")


@app.post("/import/commit")
async def import_commit_v0130(job_id: str = Form(...)) -> RedirectResponse:
    try:
        start_import_commit(get_paths(), job_id)
        return redirect(f"/import?job_id={quote(job_id)}")
    except ImportV0130Error as exc:
        return redirect(f"/import?job_id={quote(job_id)}&message={quote(str(exc))}")
...
async def save_item(
    ...
    try:
        ...
        try:
            saved_for_pricing = db.get_item(paths.db_file, item_id)
            if saved_for_pricing and saved_for_pricing.get("title"):
                record_review_pricing(
                    item_id=item_id,
                    batch_id=batch_id,
                    ...
                    previous_price_cents=current.get("price_cents"),
                    item_revision=int(saved_for_pricing["record_revision"]),
                    actor=str(getattr(request.state, "username", "") or "local-operator"),
                )
        except Exception as exc:
            LOGGER.exception("Pricing disposition could not be recorded after Item save for %s: %s", item_id, exc)
        if values["title"] and (
...
@app.post("/batch-editor/batch-name")
async def batch_editor_batch_name(request: Request) -> RedirectResponse:
    paths = get_paths()
    form = await request.form()
    batch_id = str(form.get("batch_id") or "")
    try:
        update_batch_display_name(paths.db_file, batch_id, str(form.get("display_name") or ""))
        notice = "Batch display name updated. The source folder and immutable Batch ID were not changed."
    except Exception as exc:
        notice = f"Batch name was not updated: {exc}"
    return redirect(f"/batch-editor?batch_id={quote(batch_id)}&notice={quote(notice)}")
...
def normalize_editor_value(field: str, value: Any) -> Any:
    ...
    if field == "location":
        return normalize_import_location(value)
...
async def api_update_item(item_id: str, request: Request) -> JSONResponse:
    ...
    try:
        ...
        updates = {field: value, "working_source": "BATCH_EDITOR"}
        if field == "location":
            updates["shelf"] = value or ""
        db.update_item(
            paths.db_file,
            item_id,
            updates,
...
def _shopify_item_rows(paths: DataPaths, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not items:
        return []
    ...
    with db.connect(paths.db_file) as connection:
        sync_rows = {
            ...
            for row in connection.execute(
                f"SELECT * FROM shopify_sync WHERE item_id IN ({placeholders})",
                identifiers,
            ).fetchall()
        }
    ...
    for item in items:
        row = dict(item)
        sync = sync_rows.get(str(item["item_id"]), {})
        row["shopify_sync"] = sync
        row["shopify_status"] = str(
            sync.get("product_status") or item.get("upload_status") or "NOT_UPLOADED"
        ).upper()
        row["sync_state"] = str(sync.get("sync_state") or "NOT_SYNCED").upper()
        try:
            row["shopify_conflicts"] = json.loads(str(sync.get("conflict_json") or "{}"))
        except json.JSONDecodeError:
            row["shopify_conflicts"] = {}
        result.append(row)
    return result
...
async def publish_page(
    ...
    raw_items = item_page["items"]
    items = _shopify_item_rows(paths, raw_items)
    item_page = {**item_page, "items": items}
    ...
    selected_batch_row = next((row for row in batches if row["batch_id"] == batch_id), None)
    jobs = list_shopify_jobs(paths.db_file, batch_id, limit=20) if batch_id else []
    selected_job = get_shopify_job(paths.db_file, job_id) if job_id else None
    if selected_job and str(selected_job.get("batch_id")) != batch_id:
        selected_job = None
    drafts_url = (
        f"https://{config.store_domain}/admin/products?status=DRAFT"
        if config.store_domain else ""
    )
    live_url = (
        f"https://{config.store_domain}/admin/products?status=ACTIVE"
        if config.store_domain else ""
    )
    return TEMPLATES.TemplateResponse(
        request,
        ...
        context(
            ...
            checkpoints=db.list_batch_checkpoints(paths.db_file, batch_id) if batch_id else [],
            shopify_jobs=jobs,
            shopify_job=selected_job,
            shopify_drafts_url=drafts_url,
            shopify_live_url=live_url,
            shopify_configured=not bool(config.problems()),
            shopify_publication_configured=bool(config.publication_id),
            notice=notice,
            message=message,
        ),
    )
...
async def publish_create_job(request: Request) -> Any:
    form = await request.form()
    ...
    try:
        ...
        if request.headers.get("x-requested-with") == "fetch":
            return JSONResponse({"ok": False, "error": str(exc)}, status_code=400)
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&message={quote(str(exc))}"
        )
    target = f"/publish?batch_id={quote(batch_id)}&job_id={quote(job_id)}"
    if request.headers.get("x-requested-with") == "fetch":
        return JSONResponse({"ok": True, "job_id": job_id, "redirect": target})
    return redirect(target)


@app.get("/publish/job/{job_id}")
async def publish_job_status(job_id: str) -> JSONResponse:
    job = get_shopify_job(get_paths().db_file, job_id)
    if job is None:
        raise HTTPException(404, "Shopify job not found")
    config = ShopifyConfig.from_env()
    return JSONResponse(
        {
            **job,
...
@app.post("/publish/job/{job_id}/resume")
async def publish_resume_job(job_id: str, batch_id: str = Form(...)) -> RedirectResponse:
    try:
        resume_shopify_job(get_paths(), job_id)
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&job_id={quote(job_id)}&notice="
            + quote("Shopify job resumed safely.")
        )
    except ShopifyJobError as exc:
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&job_id={quote(job_id)}&message={quote(str(exc))}"
        )


@app.post("/publish/batch/rename")
async def publish_rename_batch(
    batch_id: str = Form(...), display_name: str = Form(...)
) -> RedirectResponse:
    try:
        update_batch_display_name(get_paths(), batch_id, display_name)
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&notice={quote('Batch name updated.')}"
        )
    except Exception as exc:
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&message={quote(str(exc))}"
...
@app.post("/publish/batch/archive")
async def publish_archive_batch(
    batch_id: str = Form(...), confirmation: str = Form("")
) -> RedirectResponse:
    if confirmation.strip() != "ARCHIVE BATCH":
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&message="
            + quote("Type ARCHIVE BATCH exactly to archive the local batch.")
        )
    with db.transaction(get_paths().db_file) as connection:
        connection.execute(
            "UPDATE batches SET status='ARCHIVED' WHERE batch_id=?", (batch_id,)
        )
    _event(
        "shopify", "shopify.batch_archived", batch_id=batch_id,
        status="ARCHIVED", outcome="LOCAL_ONLY", retention_class="BUSINESS",
    )
    return redirect(
        f"/publish?batch_id={quote(batch_id)}&notice="
        + quote("Batch archived locally. Linked Shopify products were not changed.")
    )


@app.post("/publish/batch/restore")
async def publish_restore_batch(batch_id: str = Form(...)) -> RedirectResponse:
    with db.transaction(get_paths().db_file) as connection:
        connection.execute(
            "UPDATE batches SET status='IMPORTED' WHERE batch_id=? AND status IN ('ARCHIVED','DELETED')",
            (batch_id,),
        )
    return redirect(
        f"/publish?batch_id={quote(batch_id)}&notice={quote('Batch restored locally.')}"
    )


@app.post("/publish/batch/delete")
async def publish_delete_batch(
    batch_id: str = Form(...), confirmation: str = Form("")
) -> RedirectResponse:
    if confirmation.strip() != "DELETE BATCH":
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&message="
            + quote("Type DELETE BATCH exactly to remove the local batch from active operation.")
        )
    with db.transaction(get_paths().db_file) as connection:
        active = connection.execute(
            """SELECT COUNT(*) FROM shopify_publish_jobs
                 WHERE batch_id=? AND status IN ('QUEUED','RUNNING','RETRYING','PAUSED')""",
            (batch_id,),
        ).fetchone()[0]
        if active:
            return redirect(
                f"/publish?batch_id={quote(batch_id)}&message="
                + quote("This batch has an active Shopify job. Wait for it to finish before removing the batch.")
            )
        connection.execute(
            "UPDATE batches SET status='DELETED' WHERE batch_id=?", (batch_id,)
        )
    _event(
        "shopify", "shopify.batch_deleted", batch_id=batch_id,
        status="DELETED", outcome="SOFT_DELETE", retention_class="BUSINESS",
    )
    return redirect(
        f"/publish?batch_id={quote(batch_id)}&notice="
        + quote("Batch removed from active operation. Local records and Shopify linkage history were retained and can be restored.")
    )


@app.post("/publish/batch/duplicate")
async def publish_duplicate_batch(
    batch_id: str = Form(...), confirmation: str = Form("")
) -> RedirectResponse:
    if confirmation.strip() != "DUPLICATE BATCH":
        return redirect(
            f"/publish?batch_id={quote(batch_id)}&message="
            + quote("Type DUPLICATE BATCH exactly to create an independent local copy.")
        )
    paths = get_paths()
    source = db.get_batch(paths.db_file, batch_id)
    if source is None:
        return redirect("/publish?message=" + quote("Unknown batch."))
    stamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    new_batch_id = f"{batch_id}-COPY-{stamp}"
    timestamp = db.now()
    with db.transaction(paths.db_file) as connection:
        batch_row = dict(connection.execute(
            "SELECT * FROM batches WHERE batch_id=?", (batch_id,)
        ).fetchone())
        batch_row.update({
            "batch_id": new_batch_id,
            "source_fingerprint": f"duplicate:{uuid4().hex}",
            ...
            "quarantine_reason": "",
            "display_name": f"{batch_row.get('display_name') or batch_id} Copy",
            "import_job_id": "",
        })
        columns = list(batch_row)
        connection.execute(
            f"INSERT INTO batches({','.join(columns)}) VALUES({','.join('?' for _ in columns)})",
            tuple(batch_row[column] for column in columns),
        )
        ...
        for source_item in source_items:
            item_row = dict(source_item)
            old_item_id = str(item_row["item_id"])
            sequence = int(item_row["sequence"])
            new_item_id = f"{new_batch_id}-{sequence:03d}"
            item_map[old_item_id] = new_item_id
            item_row.update({
                "item_id": new_item_id, "sku": new_item_id, "batch_id": new_batch_id,
                "upload_status": "NOT_UPLOADED", "shopify_product_id": "",
                "shopify_variant_id": "", "shopify_inventory_item_id": "",
                "shopify_admin_url": "", "last_error": "", "retry_count": 0,
                "record_revision": 0, "review_source": "DUPLICATED_BATCH",
                "working_source": "DUPLICATED_BATCH", "created_at": timestamp,
                "updated_at": timestamp,
            })
            item_columns = list(item_row)
            connection.execute(
                f"INSERT INTO items({','.join(item_columns)}) VALUES({','.join('?' for _ in item_columns)})",
                tuple(item_row[column] for column in item_columns),
            )
            connection.execute(
                "INSERT INTO shopify_sync(item_id) VALUES(?)", (new_item_id,)
            )
            for tag in connection.execute(
                "SELECT tag_id,source,recognition_result_id,acceptance_state FROM item_tags WHERE item_id=?",
                (old_item_id,),
            ).fetchall():
                connection.execute(
                    """INSERT OR IGNORE INTO item_tags(
                           item_id,tag_id,source,assigned_at,recognition_result_id,acceptance_state
                       ) VALUES(?,?,?,?,NULL,?)""",
                    (new_item_id, tag["tag_id"], "DUPLICATED_BATCH", timestamp, tag["acceptance_state"]),
                )
        source_photos = connection.execute(
            "SELECT * FROM photos WHERE batch_id=? ORDER BY stream_index", (batch_id,)
        ).fetchall()
        for source_photo in source_photos:
            photo_row = dict(source_photo)
            photo_row.pop("photo_id", None)
            photo_row["batch_id"] = new_batch_id
            old_item_id = str(photo_row.get("item_id") or "")
            photo_row["item_id"] = item_map.get(old_item_id) if old_item_id else None
            photo_columns = list(photo_row)
            connection.execute(
                f"INSERT INTO photos({','.join(photo_columns)}) VALUES({','.join('?' for _ in photo_columns)})",
                tuple(photo_row[column] for column in photo_columns),
            )
    _event(
        "shopify", "shopify.batch_duplicated", batch_id=new_batch_id,
        status="IMPORTED", outcome="LOCAL_COPY",
        detail={"source_batch_id": batch_id}, retention_class="BUSINESS",
    )
    return redirect(
        f"/publish?batch_id={quote(new_batch_id)}&notice="
        + quote("Independent local batch copy created. Shopify links were deliberately cleared to prevent duplicates.")
    )
...
async def test_shopify_settings(request: Request) -> HTMLResponse:
    ...
    config = ShopifyConfig(
        ...
        publication_id=service.value("SHOPIFY_PUBLICATION_ID", "").strip(),
    )
    ...
    try:
        ...
        test_result = {
            ...
            "publications": getattr(result, "publications", ()),
        ...
        if str(form.get("save_after_test") or "") == "true":
            ...
            try:
                ...
                if identity_changed:
                    ...
                    settings_changes["SHOPIFY_PUBLICATION_ID"] = ""
...
@app.post("/settings/shopify/publication", response_class=HTMLResponse)
async def save_shopify_publication(request: Request) -> Any:
    form = await request.form()
    publication_id = str(form.get("SHOPIFY_PUBLICATION_ID") or "").strip()
    service = get_configuration_service()
    try:
        config = ShopifyConfig.from_env()
        result = ShopifyConnectionProbe(
        ...
        selected = next(
            (publication for publication in result.publications if publication.get("id") == publication_id),
            None,
        )
        if selected is None:
            raise ConfigurationError(
                "Select a publication returned by the verified Shopify connection."
            )
        service.save_nonsecret("shopify", {"SHOPIFY_PUBLICATION_ID": publication_id})
    except (ConfigurationError, ProviderProbeError, ShopifyCredentialError, ShopifyTokenError) as exc:
        return await _render_settings(request, error=redact_text(str(exc)))
    _event(
        "shopify",
        "shopify.publication_selected",
        status="SAVED",
        outcome="SUCCESS",
        detail={"publication_id": publication_id},
        retention_class="SECURITY",
    )
    return redirect(f"/settings?message={quote('Shopify publication saved.')}#shopify")
...
async def diagnostics(
    ...
    with db.connect(paths.db_file) as connection:
        ...
        shopify_status_rows = [
            dict(row)
            for row in connection.execute(
                """SELECT COALESCE(NULLIF(product_status,''),'NOT_UPLOADED') AS state,
                          COUNT(*) AS count
                     FROM shopify_sync GROUP BY COALESCE(NULLIF(product_status,''),'NOT_UPLOADED')
                     ORDER BY state"""
            ).fetchall()
        ]
        shopify_jobs_recent = [
            dict(row)
            for row in connection.execute(
                """SELECT job_id,batch_id,action,status,total,completed,succeeded,failed,
                          skipped,current_item_id,created_at,updated_at,finished_at,error_message
                     FROM shopify_publish_jobs ORDER BY created_at DESC LIMIT 20"""
            ).fetchall()
        ]
        shopify_active_jobs = int(
            connection.execute(
                """SELECT COUNT(*) FROM shopify_publish_jobs
    ...
    try:
        shopify_token_status = ShopifyTokenManager().status()
        token_summary = {
            "mode": shopify_token_status.mode,
            "state": shopify_token_status.state,
            "expires_at": shopify_token_status.expires_at,
            "granted_scopes": shopify_token_status.granted_scopes,
            "last_refresh_at": shopify_token_status.last_refresh_at,
            "last_refresh_result": shopify_token_status.last_refresh_result,
        }
    except Exception as exc:
        token_summary = {
            "mode": shopify_config.auth_mode,
            "state": "failed",
            "expires_at": "",
            "granted_scopes": (),
            "last_refresh_at": "",
            "last_refresh_result": redact_text(str(exc), known_secrets=_configured_secrets()),
        }
    shopify_diagnostics = {
        "configured": not bool(shopify_config.problems()),
        "problems": shopify_config.problems(),
        "store_domain": shopify_config.store_domain,
        "location_id": shopify_config.location_id,
        "publication_id": shopify_config.publication_id,
        "api_version": shopify_config.api_version,
        "draft_first": shopify_config.draft_only,
        "token": token_summary,
        "status_counts": shopify_status_rows,
        "active_jobs": shopify_active_jobs,
        "recent_jobs": shopify_jobs_recent,
    }
    diagnostic_summary = {
        "version": __version__,
    ...
    return TEMPLATES.TemplateResponse(
        ...
        context(
            ...
            shopify_diagnostics=redact(
                shopify_diagnostics, known_secrets=_configured_secrets()
            ),
