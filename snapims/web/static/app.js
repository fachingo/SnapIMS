(() => {
  ...
  // Import protocol failures stay on the selected folder and open a focused
  // repair menu. Closing it never deletes or edits source photographs.
  const importRepairDialog = qs("[data-import-repair-dialog]");
  if (importRepairDialog) {
    qsa("[data-repair-close]", importRepairDialog).forEach((button) => {
      button.addEventListener("click", () => importRepairDialog.close());
    });
    qsa("[data-location-select]", importRepairDialog).forEach((select) => {
      const input = document.getElementById(select.dataset.locationInput);
      const field = input?.closest("[data-location-field]");
      const syncLocationField = () => {
        const needsLocation = select.value === "location";
        if (field) field.hidden = !needsLocation;
        if (input) input.required = needsLocation;
      };
  ...
  const selectedRows = () => rows().filter((row) =>
    !row.classList.contains("hidden") && qs(".row-select", row)?.checked
  );
    ...
    input.addEventListener("keydown", (event) => {
      ...
      if (event.key === "Enter" && input.tagName !== "TEXTAREA") {
        ...
        sendEdit(input).then((saved) => {
          ...
          const nextRow = visible[currentIndex + 1];
          if (nextRow) focusSameField(nextRow, input.dataset.field);
        });
      }
  ...
  const bulkDescriptions = {
    ...
    set_location: "Sets the selected Items to this free-text location, or unassigned when blank/None. Immutable Item IDs do not change.",
  ...
  qs("[data-fill-down]")?.addEventListener("click", async () => {
    ...
    const map = {
      ...
      description: "set_description", location: "set_location",
      tag_ids: "set_tags", review: "set_review", rare: "set_rare",
    };
  ...
  document.addEventListener("keydown", async (event) => {
    ...
    if (event.key === "F2" && activeInput) {
      event.preventDefault();
      if (activeInput.matches?.("[data-tag-picker]")) activeInput.focusTagInput?.();
      else { activeInput.focus(); activeInput.select?.(); }
    }
...
window.fetch = (input, init = {}) => {
  ...
  if (csrfToken && target.origin === pageOrigin && !["GET", "HEAD", "OPTIONS"].includes(method)) {
...
// SnapIMS v0.11.0: only a deliberate operator price edit suppresses market lookup.
(() => {
  const markTouched = (input) => {
    const form = input.closest('form');
    if (!form) return;
    const marker = form.querySelector('input[name="pricing_price_touched"]');
    if (marker) marker.value = 'true';
    const disposition = form.querySelector('select[name="pricing_disposition"]');
    if (disposition) disposition.value = 'MANUAL';
  };
  document.addEventListener('input', (event) => {
    const target = event.target;
    if (target instanceof HTMLInputElement && target.matches('[data-pricing-price-input]')) markTouched(target);
  });
  document.addEventListener('change', (event) => {
    const target = event.target;
    if (target instanceof HTMLInputElement && target.matches('[data-select-all]')) {
      const form = target.closest('form');
      if (form) form.querySelectorAll('input[name="item_ids"]').forEach((box) => { box.checked = target.checked; });
    }
  });
  document.addEventListener('submit', (event) => {
    const button = event.target.querySelector('[data-prevent-double-submit]');
    if (button) {
      button.disabled = true;
...
// SnapIMS v0.15.0: durable Import progress polling only while a job is active.
(() => {
  const root = document.querySelector('[data-import-job]');
  if (!root) return;
  const jobId = root.dataset.importJob;
  const active = new Set(['QUEUED', 'RUNNING', 'COMMIT_QUEUED', 'COMMITTING']);
  if (!active.has(root.dataset.importStatus || '')) return;
  let stopped = false;
  const poll = async () => {
    ...
    try {
      const response = await fetch(`/import/job/${encodeURIComponent(jobId)}`, {headers: {'Accept': 'application/json'}});
      if (!response.ok) throw new Error(`Import progress request failed (${response.status})`);
      ...
      setText('[data-import-stage]', job.stage || job.status);
      setText('[data-import-processed]', processed);
      setText('[data-import-total]', total);
      setText('[data-import-elapsed]', Number(job.elapsed_seconds || 0).toFixed(1));
      setText('[data-import-status-label]', String(job.status || '').replaceAll('_', ' '));
      const bar = root.querySelector('[data-import-progress]');
      if (bar) bar.style.width = `${percent}%`;
      const error = root.querySelector('[data-import-error]');
      if (error && job.error_message) {
        error.hidden = false;
      ...
      const message = root.querySelector('[data-import-error]');
      if (message) {
...
// SnapIMS v0.15.0: complete Shopify draft/live publish workflow.
(() => {
  const exactButtons = document.querySelectorAll('[data-exact-confirm]');
  exactButtons.forEach((button) => {
    const form = button.closest('form');
    const input = form?.querySelector('input[name="confirmation"]');
    const phrase = button.dataset.exactConfirm || '';
    if (!input) return;
    const sync = () => { button.disabled = input.value !== phrase; };
    input.addEventListener('input', sync);
    sync();
  });

  const expectedPhrase = (action) => ({
    PUBLISH_LIVE: 'SUBMIT',
  ...
  const management = document.querySelector('[data-management-form]');
  if (management) {
    const action = management.querySelector('[data-shopify-action]');
    const confirmation = management.querySelector('[data-management-confirm]');
    const scope = management.querySelector('[data-selection-scope]');
    const boxes = [...management.querySelectorAll('[data-product-checkbox]')];
    const syncHint = () => {
      const phrase = expectedPhrase(action?.value || '');
      if (confirmation) {
        confirmation.placeholder = phrase ? `Type ${phrase}` : 'No typed confirmation required';
        confirmation.required = Boolean(phrase);
      }
    };
    action?.addEventListener('change', syncHint);
    syncHint();
    document.querySelector('[data-select-all-products]')?.addEventListener('click', () => boxes.forEach((box) => { if (!box.closest('[data-product-row]')?.hidden) box.checked = true; }));
    document.querySelector('[data-invert-products]')?.addEventListener('click', () => boxes.forEach((box) => { if (!box.closest('[data-product-row]')?.hidden) box.checked = !box.checked; }));
    scope?.addEventListener('change', () => {
      const entireBatch = scope.value === 'ALL';
      boxes.forEach((box) => { box.disabled = entireBatch; });
    });
  }
  ...
  document.querySelectorAll('form[data-shopify-job]').forEach((form) => {
    form.addEventListener('submit', async (event) => {
      event.preventDefault();
      if (form.dataset.submitting === 'true') return;
      const data = new FormData(form);
      const action = String(data.get('action') || '').toUpperCase();
      const phrase = expectedPhrase(action);
      if (phrase && String(data.get('confirmation') || '') !== phrase) {
        window.alert(`Type ${phrase} exactly to continue.`);
        return;
      }
      if (String(data.get('selection_scope') || '') === 'SELECTED' && !data.getAll('item_id').length) {
        window.alert('Select at least one product or choose Entire batch.');
        return;
      }
      let reviewTab = null;
      if (form.hasAttribute('data-open-shopify-review')) {
        reviewTab = window.open('about:blank', 'snapims-shopify-review');
        if (reviewTab) {
          reviewTab.document.write('<title>Creating Shopify drafts…</title><main style="font-family:system-ui;padding:2rem"><h1>Creating Shopify drafts…</h1><p>This tab will open the Shopify draft-products page when SnapIMS finishes.</p></main>');
      ...
      try {
        ...
        if (reviewTab) sessionStorage.setItem(`shopify-review-${payload.job_id}`, 'open');
        window.location.assign(payload.redirect);
      } catch (error) {
        if (reviewTab) reviewTab.close();
        form.dataset.submitting = 'false';
        if (submit) { submit.disabled = false; submit.textContent = original; }
        window.alert(error.message || 'Shopify job could not be started.');
      }
    });
  });

  const root = document.querySelector('[data-shopify-job-status]');
  if (!root) return;
  const jobId = root.dataset.jobId || '';
  const action = root.dataset.jobAction || '';
  const terminalStates = new Set(['COMPLETE', 'COMPLETE_WITH_FAILURES', 'FAILED', 'CANCELLED']);
  let openedReview = false;
  const updateItems = (items) => {
    items.forEach((item) => {
      const row = root.querySelector(`[data-job-item="${CSS.escape(item.item_id)}"]`);
      if (!row) return;
      row.className = `job-item status-${String(item.status || '').toLowerCase()}`;
      const status = row.querySelector('[data-item-status]');
      if (status) status.textContent = item.status || '';
      let error = row.querySelector('[data-item-error]');
      if (item.error_message) {
        if (!error) {
          error = document.createElement('small');
          error.dataset.itemError = '';
          row.append(error);
        }
  ...
  const render = (job) => {
    const text = (selector, value) => { const node = root.querySelector(selector); if (node) node.textContent = String(value); };
    text('[data-job-status]', job.status || '');
    text('[data-job-count]', `${job.completed || 0} / ${job.total || 0}`);
    text('[data-job-rate]', `${job.items_per_minute || 0} items/min`);
    text('[data-job-eta]', job.eta_seconds == null ? 'ETA calculating…' : `ETA ${job.eta_seconds} s`);
    text('[data-job-succeeded]', job.succeeded || 0);
    text('[data-job-skipped]', job.skipped || 0);
    text('[data-job-failed]', job.failed || 0);
    text('[data-job-elapsed]', job.elapsed_seconds || 0);
    const bar = root.querySelector('[data-job-progress]');
    if (bar) bar.style.width = `${job.progress_percent || 0}%`;
    updateItems(job.items || []);
  };
  const openReviewIfReady = (job) => {
    if (openedReview || action !== 'CREATE_DRAFTS' || !terminalStates.has(job.status) || !job.drafts_url) return;
    const key = `shopify-review-${jobId}`;
    if (sessionStorage.getItem(key) === 'open') {
      openedReview = true;
      sessionStorage.removeItem(key);
      window.open(job.drafts_url, 'snapims-shopify-review');
    }
  };
  const poll = async () => {
    try {
      const response = await fetch(`/publish/job/${encodeURIComponent(jobId)}`, {headers: {'Accept': 'application/json'}});
      if (!response.ok) throw new Error(`Shopify progress request failed (${response.status})`);
      const job = await response.json();
      render(job);
      openReviewIfReady(job);
      if (!terminalStates.has(job.status)) window.setTimeout(poll, document.hidden ? 3000 : 750);
      else if (root.dataset.jobTerminal !== 'true') window.setTimeout(() => window.location.reload(), 1200);
    } catch (error) {
      const status = root.querySelector('[data-job-status]');
      if (status) status.textContent = error.message;
      window.setTimeout(poll, 3000);
    }
  };
  if (root.dataset.jobTerminal !== 'true') window.setTimeout(poll, 300);
})();

// SnapIMS v0.15.0: client-side product filtering on the current paged result set.
(() => {
  ...
  const apply = () => {
    ...
    const status = String(filter?.value || '').trim().toUpperCase();
    rows.forEach((row) => {
      const text = String(row.dataset.productSearchText || '');
      const rowStatus = String(row.dataset.productStatus || '').toUpperCase();
      row.hidden = Boolean((query && !text.includes(query)) || (status && !rowStatus.includes(status)));
    });
  };
