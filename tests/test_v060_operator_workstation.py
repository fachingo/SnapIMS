...
def test_empty_batch_home_shows_operator_instruction(data_paths) -> None:
    with TestClient(app) as client:
        response = client.get("/import")
    assert response.status_code == 200
    assert "No batch folders found" in response.text
    assert str(data_paths.batches) in response.text
    assert '"detail"' not in response.text


def test_native_operating_system_folder_picker_is_removed(data_paths) -> None:
    with TestClient(app) as client:
        response = client.post("/import/browse", data={"current_folder": ""})
