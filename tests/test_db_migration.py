...
def test_v0140_shopify_linkage_upgrades_to_publish_schema_16(
    tmp_path: Path,
) -> None:
    import shutil
    ...
    paths = DataPaths.from_root(tmp_path / "v0140-upgrade").ensure()
    shutil.copy2(fixture, paths.db_file)
    db.initialize(paths.db_file, paths=paths)
    with db.connect(paths.db_file) as connection:
        assert connection.execute("PRAGMA user_version").fetchone()[0] == 16
        item = connection.execute(
            "SELECT item_id,shopify_product_id,upload_status FROM items WHERE item_id='V0140-UPGRADE-001'"
        ).fetchone()
        assert tuple(item) == (
            "V0140-UPGRADE-001",
        ...
        sync = connection.execute(
            "SELECT product_id,last_completed_step,product_status,sync_state,conflict_json FROM shopify_sync WHERE item_id='V0140-UPGRADE-001'"
        ).fetchone()
        assert sync["product_id"] == "gid://shopify/Product/1"
        assert sync["last_completed_step"] == "complete"
        assert sync["conflict_json"] == "{}"
        assert connection.execute(
            "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='shopify_publish_jobs'"
        ).fetchone()[0] == 1
