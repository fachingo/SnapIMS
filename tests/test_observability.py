...
def test_cli_parser_supports_shopify_status_test_refresh_and_jobs() -> None:
    parser = cli.build_parser()
    assert parser.parse_args(["shopify", "status"]).shopify_command == "status"
    assert parser.parse_args(["shopify", "test"]).shopify_command == "test"
    assert parser.parse_args(["shopify", "refresh"]).shopify_command == "refresh"
    jobs = parser.parse_args(["shopify", "jobs", "--batch", "B-1", "--limit", "5"])
    assert jobs.shopify_command == "jobs"
    assert jobs.batch == "B-1"
    assert jobs.limit == 5
