...
def add_page_number(paragraph) -> None:
    ...
    run = paragraph.add_run("SnapIMS Operator Guide v0.6.0  |  ")
    run.font.size = Pt(8)
...
def build() -> None:
    ...
    p2 = doc.add_paragraph(f"Version {VERSION} - Operator Workstation Release")
    ...
    add_callout(
        doc,
        "Purpose",
        "SnapIMS converts an ordered photo stream into durable inventory records, routes uncertainty to an operator, supports fast individual or batch-wide correction, and prepares validated CSV or Shopify output.",
        "info",
    )
    add_callout(
        doc,
        "Release status",
        "Version 0.6.0 is a production-pilot candidate, not 1.0. Live AI, a real Pixel batch, one live Shopify draft, physical CSV reconciliation, and final independent acceptance remain required.",
        "warning",
    )
    doc.add_page_break()
    ...
    add_steps(doc, [
        ...
        "Run recognition, or continue with manual review when a provider is unavailable.",
        "Resolve items one at a time in Review or work across the batch in Batch Editor.",
    ...
    section(doc, "6. Batch Editor", "Batch Editor is the bulk operator workstation. It edits the same authoritative working records used by Review and Publish.", [
    ...
    section(doc, "7. Command Palette and Bulk Operations", "The command palette supplements visible controls and keeps frequent actions keyboard-accessible.", [
    ...
    section(doc, "8. CSV Round Trip and Difference Preview", "CSV remains available for external spreadsheet work, but uploaded values are never applied blindly.", [
    ...
    section(doc, "9. External Review", "A valid CSV can satisfy review for large intentionally uniform batches, but only after explicit operator confirmation.", [
    ...
    section(doc, "10. Publish", "Publish validates the authoritative working batch regardless of whether edits came from AI, Review, Batch Editor, or CSV.", [
    ...
    section(doc, "11. Diagnostics and Recovery", "Diagnostics remains deliberately compact but exposes the information required to recover failures without leaking secrets.", [
        "Confirm application version, database path, schema version, and integrity status.",
        "Check provider configuration state without displaying API keys.",
        "Inspect recognition job states and the latest failure.",
        "Compare original media bytes with preview and recognition upload bytes.",
        "Use Copy Diagnostic Summary when reporting a problem.",
        "Restart SnapIMS and confirm the active batch, filters, queue, current item, and durable edits were restored.",
    ], "15-diagnostics.png", "Diagnostics and recovery information")

    doc.add_heading("12. End-of-Batch Checklist", level=1)
    add_steps(doc, [
        ...
        "Low-confidence and flagged items were reviewed using stored evidence or the original photographs.",
    ...
    doc.core_properties.subject = "SnapIMS v0.6.0 operator workflows"
    doc.core_properties.author = "SnapIMS Project"
    doc.core_properties.keywords = "SnapIMS, inventory, VHS, operator guide, batch editor"
    doc.save(OUT)
    print(OUT)
