...
def screenshot_pages(doc: Document, title: str, filename: str, caption: str) -> None:
    ...
    for index, part in enumerate(parts, 1):
        doc.add_heading(
            title if len(parts) == 1 else f"{title} — screen {index} of {len(parts)}", level=2
        )
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.paragraph_format.space_after = Pt(2)
        ...
        cp = doc.add_paragraph(
            caption if len(parts) == 1 else f"{caption} — view {index} of {len(parts)}"
        )
        cp.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def shortcut_table(doc: Document) -> None:
    rows = [
        (
            "Ctrl/Cmd+Shift+P",
            "Open Quick Command Palette; Ctrl/Cmd+K is the browser-safe fallback.",
        ),
        (
            "Enter in Review",
            "Submit valid Title/Price/Discount once and open the next unfinished tape.",
        ),
        (
            "Arrow Up/Down",
            "Move to the same editable Batch Editor field in the previous/next visible row.",
        ),
        (
            "Arrow Left/Right",
            "Move between editable cells when the text caret is already at a boundary.",
        ),
        ...
        (
            "Ctrl/Cmd+A",
            "Select all currently visible filtered rows while the Batch Editor has focus.",
        ),
        ...
        (
            "Escape",
            "Cancel an edit/dialog, close the palette, or clear selection as context allows.",
        ),
    ]
...
def build() -> None:
    ...
    p = doc.add_paragraph("Version 0.7.0 — Keyboard Workstation and Local Movie Catalog")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.style = doc.styles["Subtitle"]
    callout(
        doc,
        "Operating model",
        "SnapIMS is an exception-handling workstation: inspect the photograph, confirm or type Title, optionally change Price/Discount, then press Enter once.",
        "info",
    )
    callout(
        doc,
        "Release boundary",
        "Version 0.7.0 is not 1.0.0. Live AI, one live Shopify draft, physical CSV verification, the real Pixel pilot, production restart, guide walkthrough and blocker review remain mandatory.",
        "warning",
    )
    picture(
        doc, "01-home-production.png", "SnapIMS 0.7.0 Home — final native-browser evidence", 6.8
    )
    doc.add_page_break()
    ...
    run.font.size = Pt(9)
    doc.add_paragraph(
        "Open http://127.0.0.1:8767. Install the optional native folder picker with sudo apt install python3-tk. Without it, SnapIMS exposes the manual-path fallback."
    )
    callout(
        doc,
        "Identity rule",
        "Never relink by title, filename, spreadsheet row, provider ID or sort order. Item ID and Batch ID are permanent.",
        "danger",
    )
    doc.add_page_break()
    ...
    doc.add_paragraph(
        "CVHS1:ITEM:NEXT is the sole normal item boundary. Time gaps never split items. ITEM:CONT remains compatibility-only."
    )
    steps(
        doc,
        [
    ...
    callout(
        doc,
        "Q1 quarantine",
        "Use Q1 for mold, damage, testing or uncertainty. Do not clear the tape for listing until the physical issue is resolved.",
        "warning",
    )
    callout(
        doc,
        "Originals",
        "Import preserves original camera images. Do not rename, edit or delete them as a normal workflow shortcut.",
        "success",
    )
    doc.add_page_break()
    ...
    screen_page(
        doc,
        "1. Import preview",
        "Preview parses the QR stream without creating durable inventory.",
        [
        ...
        "20-item Preview — not imported yet",
    )

    screen_page(
        doc,
        "2. Preserve and import",
        "Preserve creates one durable Batch ID, immutable Item IDs, preserved media and an import journal.",
        [
        ...
        "03-import-complete.png",
        "Durable Import result",
    )

    screen_page(
        doc,
        "3. Review with an empty Title",
        "An empty Title is a normal quick-field exception, not a reason for extra Enter presses.",
        [
        ...
        "04-empty-title-one-enter.png",
        "Manual Title completed through the one-Enter path",
        (
            "Routine rule",
            "Title → Enter is Approve & Next when the quick record is valid.",
            "success",
        ),
    )
    ...
    screen_page(
        doc,
        "4. Movie ambiguity is an exception",
        "The Local Movie Catalog never silently chooses between materially ambiguous titles, years or media types.",
        [
        ...
        "05-catalog-ambiguity-preserved.png",
        "Ambiguous candidate held for operator selection",
    )

    screen_page(
        doc,
        "5. Local Movie match",
        "After operator confirmation, the Movie is stored locally with source provenance and reused for future matching.",
        [
        ...
        "05b-local-movie-catalog-match.png",
        "Operator-selected Movie persisted in the local catalog",
        (
            "Local first",
            "Repeated copies search the local catalog before any external request.",
            "info",
        ),
    )
    ...
    screen_page(
        doc,
        "6. Recognition blocked and manual recovery",
        "A missing key/provider is BLOCKED before recognition, not a fabricated failed identification.",
        [
        ...
        "06-recognition-blocked-manual-recovery.png",
        "Blocked provider state with a real manual path",
    )

    screen_page(
        doc,
        "7. Price → Enter → next tape",
        "When Title exists, Price is automatically focused and selected for the most common operator correction.",
        [
        ...
        "07-price-enter-advances-once.png",
        "Price-selected one-Enter automatic advancement",
    )

    screen_page(
        doc,
        "8. Quick Command Palette",
        "The palette supplements visible controls and opens from normal pages.",
        [
    ...
    doc.add_paragraph(
        "The Batch Editor remains a smooth autosaving grid. Keyboard movement respects visible rows, filters, sorting, read-only cells and text-caret boundaries."
    )
    shortcut_table(doc)
    callout(
        doc,
        "Selection scope",
        "Ctrl/Cmd+A means currently visible filtered rows. Bulk actions must never silently include rows the operator could not reasonably know were selected.",
        "warning",
    )
    doc.add_page_break()
    screenshot_pages(
        doc,
        "9. Batch Editor keyboard reference",
        "09-batch-editor-keyboard-grid.png",
        "Keyboard grid, visible-row selection and quick-action preview",
    )

    screen_page(
        doc,
        "10. Quick actions and atomic bulk work",
        "The first nine visible actions have Ctrl/Cmd+1–9 shortcuts and use the same safe preview as a click.",
        [
        ...
        "10-batch-editor-atomic-bulk.png",
        "Atomic 20-row bulk Price operation",
        (
            "Checkpoint",
            "Destructive or large bulk actions create a rollback checkpoint before commit.",
            "success",
        ),
    )
    ...
    screen_page(
        doc,
        "11. CSV stage, diff and apply",
        "CSV remains keyed by immutable Item ID and never overwrites inventory immediately on upload.",
        [
        ...
        "11-csv-difference-preview.png",
        "Staged CSV difference preview",
    )

    screen_page(
        doc,
        "12. Shopify simulation",
        "Publish uses saved working values and structured local Movie facts; simulation remains the default safe action.",
        [
        ...
        "14-shopify-simulation-production-manual-replacement.png",
        "Production-safe Shopify simulation",
    )

    screen_page(
        doc,
        "13. Restart durability",
        "Committed inventory, Review state, CSV stages, checkpoints, import journals and catalog jobs survive a full application-process restart.",
        [
        ...
        "13-restart-durability-production.png",
        "Same batch after full process restart",
    )

    screen_page(
        doc,
        "14. Diagnostics and catalog administration",
        "Diagnostics is the operational truth source when a normal workflow cannot continue.",
        [
        ...
        "15-diagnostics-catalog.png",
        "Inventory and Local Movie Catalog diagnostics",
    )
    ...
    callout(
        doc,
        "1.0.0 remains forbidden",
        "Do not label SnapIMS 1.0.0 until the real 20-tape Pixel pilot, live AI, one live Shopify draft, physical CSV verification, production restart durability, independent guide walkthrough, browser verification and blocker review all pass.",
        "danger",
    )
    ...
    props.keywords = (
        "SnapIMS, Canada VHS, VHS, inventory, QR, local movie catalog, Wikipedia, Shopify"
    )
