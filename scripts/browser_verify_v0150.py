...
DATA = Path(os.getenv("SNAPIMS_BROWSER_DATA", "/tmp/snapims-v0150-browser-data"))
...
def _seed() -> dict[str, str]:
    ...
    for index, item in enumerate(items, start=1):
        db.update_item(
            ...
            reason="Seed publish-ready browser title",
        )
    items = db.list_items(paths.db_file, batch_id=demo.batch_id)
    source = items[0]
    db.update_item(
    ...
    large = _direct_batch(paths, 5000, "BATCH-V0150-5000")
    ...
    draft_job = "BROWSER-DRAFT-JOB"
    created = db.now()
    items = db.list_items(paths.db_file, batch_id=demo.batch_id)
    with db.transaction(paths.db_file) as connection:
        connection.execute(
            """INSERT INTO shopify_publish_jobs(
                   job_id,batch_id,action,status,selection_scope,total,completed,
                   succeeded,failed,skipped,current_item_id,confirmation_text,
                   options_json,error_message,created_at,started_at,updated_at,finished_at
               ) VALUES(?,?,'CREATE_DRAFTS','COMPLETE','ALL',?,?,?,0,0,'','','{}','',?,?,?,?)""",
            (draft_job, demo.batch_id, len(items), len(items), len(items), created, created, created, created),
        )
        for sequence, item in enumerate(items, start=1):
            product_id = f"gid://shopify/Product/{9000 + sequence}"
            variant_id = f"gid://shopify/ProductVariant/{9100 + sequence}"
            inventory_id = f"gid://shopify/InventoryItem/{9200 + sequence}"
            admin_url = f"https://browser-shop.myshopify.com/admin/products/{9000 + sequence}"
            connection.execute(
                ...
                (product_id, variant_id, inventory_id, admin_url, created, item["item_id"]),
            )
            connection.execute(
                """INSERT OR REPLACE INTO shopify_sync(
                       item_id,status,product_id,variant_id,inventory_item_id,admin_url,
                       media_count,last_synced_at,last_error,retry_count,idempotency_key,
                       last_completed_step,product_status,sync_state,last_action,conflict_json
                   ) VALUES(?,?,?,?,?,?,1,?,'',0,'','complete','DRAFT','IN_SYNC','CREATE_DRAFT','{}')""",
                (item["item_id"], "UPLOADED", product_id, variant_id, inventory_id, admin_url, created),
            )
            connection.execute(
                """INSERT INTO shopify_publish_job_items(
                       job_id,item_id,sequence,action,status,attempt_count,error_message,
                       result_json,started_at,updated_at,finished_at
                   ) VALUES(?,?,?,'CREATE_DRAFTS','SUCCESS',1,'',?,?,?,?)""",
                (
                    draft_job,
                    item["item_id"],
                    sequence,
                    json.dumps({"product_id": product_id, "admin_url": admin_url, "status": "UPLOADED_AS_DRAFT"}),
                    created,
                    created,
                    created,
                ),
            )
    return {"editor": demo.batch_id, "large": large, "draft_job": draft_job}
...
def _browser_pass(browser_type: BrowserType, executable_path: str | None, label: str, ids: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        load("/settings#shopify")
        shopify_text = page.locator("#shopify").inner_text()
        _check(results, f"{label} Shopify Client ID field", page.locator('#shopify input[name="SHOPIFY_CLIENT_ID"]:visible').count() >= 1)
        _check(results, f"{label} Shopify Client Secret field", page.locator('#shopify input[name="SHOPIFY_CLIENT_SECRET"]').count() == 1)
        _check(results, f"{label} no manual Admin token field", "Unsaved Admin API token" not in shopify_text and "Admin API token to save" not in shopify_text)
        _check(results, f"{label} draft-only safety visible", "Draft-only publishing" in shopify_text)
        page.screenshot(path=str(EVIDENCE / f"{label}-shopify-settings.png"), full_page=True)
        page.locator("#shopify").screenshot(path=str(EVIDENCE / f"{label}-shopify-section.png"))
        page.set_viewport_size({"width": 390, "height": 844})
        load("/settings#shopify")
        shopify_box = page.locator("#shopify").evaluate("""el => ({client: el.clientWidth, scroll: el.scrollWidth, right: el.getBoundingClientRect().right})""")
        shopify_overflowers = page.locator("#shopify").evaluate("""root => [...root.querySelectorAll('*')].map(el => ({tag: el.tagName, cls: el.className || '', id: el.id || '', right: el.getBoundingClientRect().right, scroll: el.scrollWidth, client: el.clientWidth})).filter(x => x.right > window.innerWidth + 2 || x.scroll > x.client + 2).sort((a,b) => Math.max(b.right,b.scroll)-Math.max(a.right,a.scroll)).slice(0,8)""")
        _check(
            results,
            f"{label} Shopify mobile fits",
            shopify_box["scroll"] <= shopify_box["client"] + 2 and shopify_box["right"] <= 392 and not shopify_overflowers,
            f"shopify={shopify_box} overflow={shopify_overflowers}",
        )
        page.screenshot(path=str(EVIDENCE / f"{label}-shopify-settings-mobile.png"), full_page=True)
        page.locator("#shopify").screenshot(path=str(EVIDENCE / f"{label}-shopify-section-mobile.png"))
        ...
        page.locator(".editor-toolbar").screenshot(path=str(EVIDENCE / f"{label}-batch-editor-controls.png"))
        ...
        publish_path = f"/publish?batch_id={ids['editor']}&simulate=true"
        load(publish_path)
        _check(results, f"{label} publish validation passed", page.get_by_text("PASSED", exact=True).count() >= 2)
        _check(results, f"{label} draft action enabled", page.get_by_role("button", name="Create Shopify Drafts").is_enabled())
        submit_input = page.locator('form[data-confirm-phrase="SUBMIT"] input[name="confirmation"]')
        submit_button = page.get_by_role("button", name="Publish Drafts Live")
        _check(results, f"{label} live publish initially disabled", submit_button.is_disabled())
        submit_input.fill("SUBMIT")
        _check(results, f"{label} exact SUBMIT enables live publish", submit_button.is_enabled())
        fast_track = page.locator('details.danger-zone').filter(has_text="Fast track")
        fast_track.locator("summary").click()
        direct_input = fast_track.locator('input[name="confirmation"]')
        direct_button = fast_track.get_by_role("button", name="Submit Entire Batch Live")
        direct_input.fill("SUBMIT LIVE")
        _check(results, f"{label} exact SUBMIT LIVE enables direct publish", direct_button.is_enabled())
        _check(results, f"{label} product bulk management present", page.locator('[data-management-form]').count() == 1)
        _check(results, f"{label} permanent delete warning present", "Permanent Shopify deletion cannot be restored" in page.content())
        page.screenshot(path=str(EVIDENCE / f"{label}-publish-workflow.png"), full_page=True)

        load(f"/publish?batch_id={ids['editor']}&job_id={ids['draft_job']}")
        _check(results, f"{label} durable draft job complete", page.locator('[data-job-status]').inner_text() == "COMPLETE")
        _check(results, f"{label} created drafts link visible", page.get_by_role("link", name="Open Created Drafts").count() == 1)
        _check(results, f"{label} per-product Shopify links visible", page.get_by_role("link", name="Open in Shopify").count() == 5)
        _check(results, f"{label} product GIDs persisted", page.locator('code').filter(has_text="gid://shopify/Product/").count() >= 5)
        page.screenshot(path=str(EVIDENCE / f"{label}-publish-drafts-complete.png"), full_page=True)

        page.set_viewport_size({"width": 390, "height": 844})
        load(publish_path)
        publish_body_width = page.evaluate("document.body.scrollWidth")
        publish_viewport_width = page.evaluate("window.innerWidth")
        _check(results, f"{label} mobile publish body fits", publish_body_width <= publish_viewport_width + 2, f"body={publish_body_width} viewport={publish_viewport_width}")
        _check(results, f"{label} mobile publish controls visible", page.get_by_role("button", name="Create Shopify Drafts").count() == 1)
        page.screenshot(path=str(EVIDENCE / f"{label}-mobile-publish.png"), full_page=True)
        page.set_viewport_size({"width": 1440, "height": 1000})

        load("/diagnostics")
        _check(results, f"{label} Shopify diagnostics visible", "Shopify publication" in page.content() and "Durable Shopify jobs" in page.content())
        page.screenshot(path=str(EVIDENCE / f"{label}-diagnostics-shopify.png"), full_page=True)
        ...
        if form.count() == 1:
            ...
            page.locator(".preview-summary").screenshot(path=str(EVIDENCE / f"{label}-import-preview-summary.png"))
    finally:
        context.close()
        server_client.close()
        browser.close()
...
def main() -> int:
    ...
    env.update({
        ...
        "OPENAI_API_KEY": "",
        "SHOPIFY_STORE_DOMAIN": "browser-shop.myshopify.com",
        "SHOPIFY_ADMIN_ACCESS_TOKEN": "browser-static-token",
        "SHOPIFY_CLIENT_ID": "",
        "SHOPIFY_CLIENT_SECRET": "",
        "SHOPIFY_LOCATION_ID": "gid://shopify/Location/8334999560",
